...
function MainDocument({ lessonData }) {
  const featured = lessonData || {};
  const featuredUrl =
    featured.featured_url || "https://example.com/giao-trinh.pdf";
    ...
    <div className="bg-[#1e293b] rounded-2xl border border-slate-600 p-6 shadow-sm">
      ...
      <div className="border border-purple-600/40 bg-gradient-to-r from-[#29264a] to-[#32263d] rounded-xl p-6 mb-6">
          ...
          <div className="flex items-start">
            ...
            <div>
              <h4 className="font-bold text-lg text-white">
                {featured.featured_title || "Giáo trình An ninh mạng cơ bản"}
              </h4>
              <p className="text-sm text-slate-300">
                {featured.featured_info || "PDF • 45 trang • Cập nhật 2024"}
              </p>
            </div>
          </div>
          <button className="p-2 bg-slate-600 border border-slate-500 rounded-lg hover:bg-slate-500 transition-colors">
        ...
        <div className="bg-slate-600/50 border border-slate-500 rounded-xl p-4 mb-4">
          <p className="text-sm text-slate-200 leading-relaxed">
            {featured.featured_description ||
              "Tài liệu này cung cấp kiến thức nền tảng về an ninh mạng, bao gồm các khái niệm cơ bản, mối đe dọa phổ biến, và các biện pháp bảo vệ. Phù hợp cho người mới bắt đầu học về bảo mật."}
          </p>
        </div>
        ...
        <a
          ...
          className="w-full py-3 px-6 bg-gradient-to-r from-purple-600 to-pink-500 hover:from-pink-500 hover:to-purple-600 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transition-all flex items-center justify-center"
        >
          <BookOpen className="w-4 h-4 mr-2" />
          Đọc tài liệu
        </a>
      ...
      {/* Additional Materials */}
      <div className="grid grid-cols-2 gap-4">
        {featured.additional_materials?.length > 0 ? (
          featured.additional_materials.map((material, index) => (
            <div
              key={index}
              className="bg-slate-600/50 border border-slate-500 rounded-xl p-4 flex items-center justify-between"
            >
              <div className="flex items-center">
                <div className="w-10 h-10 bg-purple-600/20 rounded-lg flex items-center justify-center mr-3">
                  <FileText className="w-5 h-5 text-purple-400" />
...
function LearningObjectives({ lessonData }) {
      ...
      <div className="space-y-4">
        {objectives.length > 0 ? (
          objectives.map((objective, index) => (
            <div key={objective.number || index} className="flex items-start">
              <div className="w-8 h-8 bg-gradient-to-r from-purple-600 to-pink-500 rounded-xl flex items-center justify-center mr-4 mt-0.5">
          ...
          <p className="text-slate-300">
            Chưa có mục tiêu học tập được định nghĩa.
          </p>
        )}
      </div>
    </div>
...
export function CourseContent({ lessonData }) {
  return (
    <div className="space-y-8">
      <MainDocument lessonData={lessonData} />
      <LearningObjectives lessonData={lessonData} />
    </div>
  );
}
