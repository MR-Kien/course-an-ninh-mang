...
function CourseDetails({ courseData }) {
  ...
  const isCompleted =
    (course.completed_count || 0) === (course.total_count || 0);
...
function AdditionalResources() {
  // Hard-coded, có thể fetch từ backend sau
  const resources = [
    "NIST Cybersecurity Framework",
    "OWASP Top 10",
    "ISO 27001 Guidelines",
    ...
    <div className="bg-[#1e293b] rounded-2xl border border-slate-600 p-6 shadow-sm mb-4">
      ...
      <div className="space-y-3">
        {resources.map((resource, index) => (
          <div
            key={index}
            className="flex items-center justify-between p-2 rounded-lg hover:bg-slate-600/50 transition-colors cursor-pointer"
          >
            <div className="flex items-center">
              {index === 3 ? (
                <FileText className="w-4 h-4 text-slate-400 mr-3" />
...
function LearningProgress({ courseData }) {
  // Simplified: Dựa trên progress từ backend
  const progressItems = [
    { task: "Đọc tài liệu", completed: (courseData?.completed_count || 0) > 0 },
    { task: "Làm bài tập", completed: false },
    {
      task: "Kiểm tra",
      completed: courseData?.test?.status === "completed" || false,
    },
  ];
...
export function CourseInfo({ courseData }) {
  return (
    <div className="space-y-4">
      ...
      <AdditionalResources />
      <LearningProgress courseData={courseData} />
    </div>
  );
}
